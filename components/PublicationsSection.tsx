...
const publicationsData = [
  {
    ...
    conference:
      "International Conference on Next-Gen Engineering & Technology (ICNEXT-2026)",
    isbn: "978-93-5768-920-5",
    authors:
      "Bhanu Kiran R, Bindan N, Harshitha L, Harshitha N S and Guided by Dr. Vasudeva R",
...
/* ─────────────────────────────────────────────────────────────────
   Card content — just the visual card, no positioning/stacking logic
───────────────────────────────────────────────────────────────── */
function CardContent({
  ...
  return (
    <div className="relative rounded-3xl overflow-hidden group cursor-default shadow-2xl">
      {/* Card Background */}
      <div
        className="absolute inset-0 rounded-3xl"
        style={{
          background:
            "linear-gradient(135deg, rgba(15,15,23,0.98) 0%, rgba(10,10,18,0.99) 100%)",
          ...
          <div>
            ...
            <div className="space-y-1.5 mb-4">
              <p className="text-xs text-text-muted font-mono flex items-start gap-2">
                <span
                  className="w-3 h-px mt-2 shrink-0"
...
/* ─────────────────────────────────────────────────────────────────
   ...
   CRITICAL STRUCTURE for sticky stacking:
   All sticky cards MUST share the SAME parent container.
   Spacer divs are SIBLINGS of the sticky cards (not parents).
   
   ┌──────────────────────────────────────────────┐
   │  SHARED CONTAINER (relative)                  │
   │  ┌──────────────────────────────────────────┐ │
   │  │  Card 1 (md:sticky, top: 10vh, z: 1)    │ │  ← sticks here
   │  └──────────────────────────────────────────┘ │
   │  ┌──────────────────────────────────────────┐ │
   │  │  Spacer (height: 50vh on desktop)        │ │  ← creates scroll travel
   │  └──────────────────────────────────────────┘ │
   │  ┌──────────────────────────────────────────┐ │
   │  │  Card 2 (md:sticky, top: 10vh+28, z: 2) │ │  ← slides over Card 1
   │  └──────────────────────────────────────────┘ │
   └──────────────────────────────────────────────┘
───────────────────────────────────────────────────────────────── */
export default function PublicationsSection() {
  ...
  // Scale/opacity for Card 1 (shrinks & fades as Card 2 overlaps)
  const scale0 = useTransform(scrollYProgress, [0.3, 0.55], [1, 0.92]);
  const opacity0 = useTransform(scrollYProgress, [0.3, 0.55], [1, 0.25]);

  return (
    <section
      id="publications"
      ref={sectionRef}
      className="relative w-full bg-bg-primary py-28"
    >
      {/* Ambient glows */}
      <div
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6">
        ...
        >
          ...
          <p className="text-text-secondary mt-8 text-base max-w-xl mx-auto">
            Peer-reviewed research in AI/ML applications and agricultural
            technology, presented at national and international conferences.
          </p>
        </motion.div>

        {/* ══════════════════════════════════════════════════════════
            STICKY CARD STACK — all cards share this SINGLE container
           ══════════════════════════════════════════════════════════ */}
          ...
          {publicationsData.map((pub, idx) => {
            ...
            return (
              <React.Fragment key={pub.id}>
                {/* STICKY CARD — sticks at top, higher index = higher z */}
                <motion.div
                  className="relative md:sticky"
                  style={{
                    top: `calc(10vh + ${idx * 28}px)`,
                    zIndex: idx + 1,
                    // Scale & opacity animation for Card 1 only
                    scale: idx === 0 ? scale0 : 1,
                    opacity: idx === 0 ? opacity0 : 1,
                ...
                >
                  ...
                  >
                    <CardContent pub={pub} index={idx} accent={accent} />
                  </motion.div>
                </motion.div>

                {/* SPACER — creates scroll travel so the next card can slide over.
                    Only between cards, not after the last one.
                    Sibling of sticky card, NOT parent. Hidden on mobile. */}
                {!isLast && (
                  <div
                ...
                {!isLast && <div className="block md:hidden h-8" />}
              </React.Fragment>
            );
          })}
        </div>
